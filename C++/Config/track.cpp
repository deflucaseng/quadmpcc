...
TrackFull Track::getTrackFull() const {
  return {X_,       Y_,      X_inner_, Y_inner_, X_outer_,
          Y_outer_, n_left_, n_right_, s_,       v_};
}
...
// {
//     return {1*Eigen::Map<Eigen::VectorXd>(X.data(), X.size()),
//     1*Eigen::Map<Eigen::VectorXd>(Y.data(), Y.size()),
//             1*Eigen::Map<Eigen::VectorXd>(X_inner.data(), X_inner.size()),
//             1*Eigen::Map<Eigen::VectorXd>(Y_inner.data(), Y_inner.size()),
//             1*Eigen::Map<Eigen::VectorXd>(X_outer.data(), X_outer.size()),
//             1*Eigen::Map<Eigen::VectorXd>(Y_outer.data(), Y_outer.size())};
// }
}  // namespace mpcc
