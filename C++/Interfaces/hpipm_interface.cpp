...
namespace mpcc {
void HpipmInterface::setDynamics(std::array<Stage, N + 1> *stages,
                                 const State &x0) {
  b0_ =
      ((*stages)[0].lin_model.A * stateToVector(x0) + (*stages)[0].lin_model.g);
  for (int i = 0; i < N; i++) {
    if (i == 0) {
      hA_[i] = nullptr;
...
void HpipmInterface::setBounds(std::array<Stage, N + 1> *stages,
                               const State &x0) {
  nbu_[0] = 0;
  ...
  for (int i = 1; i <= N; i++) {
    ...
    nbu_[i] = 0;
    for (int j = 0; j < NU; j++) {
      if ((*stages)[i].l_bounds_u(j) > -INF &&
          (*stages)[i].u_bounds_u(j) < INF) {
        nbu_[i]++;
    ...
    nbx_[i] = 0;
    for (int j = 0; j < NX; j++) {
      if ((*stages)[i].l_bounds_x(j) > -INF &&
          (*stages)[i].u_bounds_x(j) < INF) {
        nbx_[i]++;
...
std::array<OptVariables, N + 1> HpipmInterface::Solve(int *status) {
  ...
  d_ocp_qp_set_all(hA_, hB_, hb_, hQ_, hS_, hR_, hq_, hr_, hidxbx_, hlbx_,
                   hubx_, hidxbu_, hlbu_, hubu_, hC_, hD_, hlg_, hug_, hZl_,
                   hZu_, hzl_, hzu_, hidxs_, hlls_, hlus_, &qp);
  ...
  double *u = reinterpret_cast<double *>(malloc(nu_max * sizeof(double)));
  //    printf("\nu = \n");
  for (ii = 0; ii <= N; ii++) {
  ...
  double *x = reinterpret_cast<double *>(malloc(nx_max * sizeof(double)));
  //    printf("\nx = \n");
  for (ii = 0; ii <= N; ii++) {
  ...
  //    printf("\nipm return = %d\n", hpipm_return);
  //    printf("\nipm residuals max: res_g = %e, res_b = %e, res_d = %e, res_m =
