...
class HpipmInterface : public SolverInterface {
 public:
  std::array<OptVariables, N + 1> solveMPC(std::array<Stage, N + 1> *stages,
                                           const State &x0,
                                           int *status) override;
 ...
 private:
  ...
  // min_x,u sum_k=0^N 1/2*[x_k;u_k]^T*[Q_k , S_k; S_k^T , R_k]*[x_k;u_k] +
  // [q_k; r_k]^T*[x_k;u_k]
  ...
  int *hidxbx_[N + 1];   // hidxbx[k] = {0,1,2,...,nx} for bounds on all inputs
                         // and states
  ...
  int *hidxbu_[N + 1];   // hidxbuk] = {0,1,2,...,nu} for bounds on all inputs
                         // and states
  ...
  // upper bounds min_x,u sum_k=0^N 1/2*[s_lower,k;s_upper,k]^T*[Z_lower,k , 0;
  // 0 , Z_upper,k]*[s_lower,k;s_upper,k] + [z_lower,k;
