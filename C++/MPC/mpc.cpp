...
void MPC::setStage(const State &xk, const Input &uk, const State &xk1,
                   const int time_step) {
  ...
  stages_[time_step].cost_mat =
      normalizeCost(cost_.getCost(track_, xk_nz, uk, time_step));
  stages_[time_step].lin_model =
      normalizeDynamics(model_.getLinModel(xk_nz, uk, xk1_nz));
  stages_[time_step].constrains_mat =
      normalizeCon(constraints_.getConstraints(track_, xk_nz, uk));
  ...
  if (time_step == N) {
    ...
    stages_[time_step].u_bounds_x =
        normalization_param_.T_x_inv *
        (bounds_.addVeloUX(xk_nz, bounds_.getBoundsUX(xk_nz),
                           terminal_v_constraint));
  } else {
    stages_[time_step].u_bounds_x =
...
std::array<OptVariables, N + 1> MPC::sqpSolutionUpdate(
  ...
  // TODO(alex) use line search and merit function
...
MPCReturn MPC::runMPC(const State &x0) {
  ...
  if (valid_initial_guess_)
    ...
    generateNewInitialGuess(x_mpc);

  // TODO(alex): this is one approach to handle solver errors, works well in
  // simulation
  n_no_solves_sqp_ = 0;
