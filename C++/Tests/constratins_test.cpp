...
int testAlphaConstraint(const PathToJson &path) {
  ...
  std::cout << constraints_0.dl(2) << "<=" << constraints_0.C.row(2) * xk1_vec
            << "<=" << constraints_0.du(2) << std::endl;

  if ((constraints_0.dl(2) <= constraints_0.C.row(2) * xk1_vec &&
       constraints_0.C.row(2) * xk1_vec <= constraints_0.du(2)) ^
      (-param.max_alpha <= alpha_f && alpha_f <= param.max_alpha)) {
    return 0;
  }

  ConstrainsMatrix constraints_1 = constraints.getConstraints(track, xk1, uk1);
  std::cout << constraints_1.dl(2) << "<=" << constraints_1.C.row(2) * xk1_vec
            << "<=" << constraints_1.du(2) << std::endl;

  if ((constraints_1.dl(2) <= constraints_1.C.row(2) * xk1_vec &&
       constraints_1.C.row(2) * xk1_vec <= constraints_1.du(2)) ^
      (-param.max_alpha <= alpha_f && alpha_f <= param.max_alpha)) {
    return 0;
  }
  ...
  std::cout << "true alpha " << alpha_f << std::endl;
  std::cout << constraints_2.dl(2) << "<=" << constraints_2.C.row(2) * xk2_vec
            << "<=" << constraints_2.du(2) << std::endl;

  if ((constraints_2.dl(2) <= constraints_2.C.row(2) * xk2_vec &&
       constraints_2.C.row(2) * xk2_vec <= constraints_2.du(2)) ^
      (-param.max_alpha <= alpha_f && alpha_f <= param.max_alpha)) {
    return 0;
  }
...
int testTireForceConstraint(const PathToJson &path) {
  ...
  tireForce = std::sqrt(std::pow(param.e_long * f_rear.F_x, 2) +
                        std::pow(f_rear.F_y, 2));
  maxForce = param.e_eps * param.Dr;

  ConstrainsMatrix constraints_mat =
      constraints.getConstraints(track, xk2, uk2);

  //    std::cout << C.row(1) <<   std::endl;
  std::cout << 0 << "<=" << tireForce << "<=" << maxForce << std::endl;
  std::cout << constraints_mat.dl(1)
            << "<=" << constraints_mat.C.row(1) * xk1_vec
            << "<=" << constraints_mat.du(1) << std::endl;
...
int testTrackConstraint(const PathToJson &path) {
  ...
  ConstrainsMatrix constraints_mat =
      constraints.getConstraints(track, xk0, uk0);

  //    std::cout << "inside track case " << dl(0) << "<=" << C.row(0)*xk0 <<
  //    "<=" << du(0) << std::endl; std::cout << "right side case "   << dl(0)
  //    << "<=" << C.row(0)*xk1 <<  "<=" << du(0) << std::endl; std::cout <<
  //    "left side case "    << dl(0) << "<=" << C.row(0)*xk2 <<  "<=" << du(0)
  //    << std::endl;

  if (!(constraints_mat.dl(0) <= constraints_mat.C.row(0) * xk0_vec &&
        constraints_mat.C.row(0) * xk0_vec <= constraints_mat.du(0))) {
    return 0;
  }
  if (constraints_mat.dl(0) <= constraints_mat.C.row(0) * xk1_vec &&
      constraints_mat.C.row(0) * xk1_vec <= constraints_mat.du(0)) {
    return 0;
  }
  if (constraints_mat.dl(0) <= constraints_mat.C.row(0) * xk2_vec &&
      constraints_mat.C.row(0) * xk2_vec <= constraints_mat.du(0)) {
    return 0;
  }
