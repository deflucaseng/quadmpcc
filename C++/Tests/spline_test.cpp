...
int testSpline() {
  ...
  std::cout << "mean fit derivative error: " << (dyt - dytt).sum() / NV
            << std::endl;

  std::cout << "mean fit second derivative error: " << (ddyt - ddytt).sum() / NV
            << std::endl;

  // return true if fit is good enough
  if ((yt - ytt).sum() / NV <= 1e-4 && (dyt - dytt).sum() / NV <= 1e-3 &&
      (ddyt - ddytt).sum() / NV <= 1e-1) {
    return 1;
  } else {
    return 0;
  }
...
int testArcLengthSpline(const PathToJson &path) {
  ...
  for (int i = 0; i < NV; i++) {
    ...
    error(i) = std::sqrt(std::pow(pos(0) - std::cos(phiv(i)), 2) +
                         std::pow(pos(1) - std::sin(phiv(i)), 2));
  }
  std::cout << "norm of error = " << error.norm() << std::endl;

  if (error.norm() / static_cast<double>(NV) <= 0.03)
    return 1;
  else
    return 0;
