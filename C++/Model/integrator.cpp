...
Integrator::Integrator(double Ts, const PathToJson &path) : model_(Ts, path) {}
...
State Integrator::RK4(const State &x, const Input &u, const double ts) const {
  ...
  const StateVector x_next =
      x_vec + ts * (k1 / 6. + k2 / 3. + k3 / 3. + k4 / 6.);
...
State Integrator::simTimeStep(const State &x, const Input &u,
                              const double ts) const {
  // integrate time step
  State x_next = x;
  const int integration_steps = static_cast<int>(ts / fine_time_step_);
