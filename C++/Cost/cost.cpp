...
TrackPoint Cost::getRefPoint(const BoostSplines &track, const State &x) const {
  ...
  if (std::fabs(dtheta_ref_nom) < 1e-7) dtheta_ref_nom = 0;
  if (std::fabs(dtheta_ref_denom) < 1e-7) dtheta_ref_denom = 1e-7;
...
CostMatrix Cost::getContouringCost(const BoostSplines &track, const State &x,
                                   const int k) const {
...
CostMatrix Cost::getHeadingCost(const BoostSplines &track, const State &x,
                                int k) const {
...
CostMatrix Cost::getCost(const BoostSplines &track, const State &x,
  ...
  // TODO(alex) do this properly directly in the differnet functions computing
  // the cost
