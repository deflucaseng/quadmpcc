...
void BoostSplines::genPathSplines(const Eigen::VectorXd &s,
                                  const Eigen::VectorXd &X,
                                  const Eigen::VectorXd &Y) {
  double diff_s = s[1] - s[0];
  boost::math::interpolators::cardinal_cubic_b_spline<double> temp_x(
      X.begin(), X.end(), 0.0, diff_s);
  boost::math::interpolators::cardinal_cubic_b_spline<double> temp_y(
      Y.begin(), Y.end(), 0.0, diff_s);
  ...
  path_data_.n_points = static_cast<int>(s.size());
  length_s_ = s.tail(1)(0);
}

void BoostSplines::genBoarderSplines(const Eigen::VectorXd &s,
                                     const Eigen::VectorXd &n_left,
                                     const Eigen::VectorXd &n_right) {
  double diff_s = s[1] - s[0];
  boost::math::interpolators::cardinal_cubic_b_spline<double> temp_n_left(
      n_left.begin(), n_left.end(), 0.0, diff_s);
  boost::math::interpolators::cardinal_cubic_b_spline<double> temp_n_right(
      n_right.begin(), n_right.end(), 0.0, diff_s);

  spline_n_left_ = temp_n_left;
  spline_n_right_ = temp_n_right;
}

void BoostSplines::genVeloSplines(const Eigen::VectorXd &s,
                                  const Eigen::VectorXd &v) {
  double diff_s = s[1] - s[0];
  boost::math::interpolators::cardinal_cubic_b_spline<double> temp_v(
      v.begin(), v.end(), 0.0, diff_s);
...
double BoostSplines::projectOnSpline(const State &x) const {
  ...
  for (int i = 0; i < 20; i++) {
    ...
    Eigen::Vector2d dds_path = {spline_x_.double_prime(s_opt),
                                spline_y_.double_prime(s_opt)};
