...
void ArcLengthSpline::setData(const Eigen::VectorXd &X_in,
...
void ArcLengthSpline::setRegularData(const Eigen::VectorXd &X_in,
                                     const Eigen::VectorXd &Y_in,
                                     const Eigen::VectorXd &s_in) {
  // set final x-y data if x and y have same length
  // x-y points are space such that they are very close to arc length
  // parametrized
  if (X_in.size() == Y_in.size()) {
    path_data_.X = X_in;
    path_data_.Y = Y_in;
...
Eigen::VectorXd ArcLengthSpline::compArcLength(
    const Eigen::VectorXd &X_in, const Eigen::VectorXd &Y_in) const {
  ...
  for (int i = 0; i < n_points - 1; i++) {
    ...
    dy = Y_in(i + 1) - Y_in(i);
    dist = std::sqrt(dx * dx +
                     dy * dy);  // dist is straight line distance between points
...
PathData ArcLengthSpline::resamplePath(const CubicSpline &initial_spline_x,
                                       const CubicSpline &initial_spline_y,
                                       const double total_arc_length) const {
  // re-sample arc length parametrized X-Y spline path with N_spline data points
  // using equidistant arc length values
  // successively re-sample, computing the arc length and then fit the path
  // should result in close to equidistant points w.r.t. arc length
...
RawPath ArcLengthSpline::outlierRemoval(
    const Eigen::VectorXd &X_original,
    const Eigen::VectorXd &Y_original) const {
  // remove points which are not at all equally spaced, to avoid fitting
  // problems

  // compute mean distance between points and then process the points such that
  // points are not closer than 0.75 the mean distance
  ...
  for (int i = 1; i < n_points - 1; i++) {
    // compute distance between currently checked point and the one last added
    // to the new X-Y path
    dx = X_original(i) - X_original(j);
    dy = Y_original(i) - Y_original(j);
    dist = std::sqrt(dx * dx + dy * dy);
    // if this distance is smaller than 0.7 the mean distance add this point to
    // the new X-Y path
...
void ArcLengthSpline::fitSpline(const Eigen::VectorXd &X,
  ...
  // 1. re-sample
  first_refined_path =
      resamplePath(first_spline_x, first_spline_y, total_arc_length);
  ...
  // 2. re-sample
  second_refined_path =
      resamplePath(second_spline_x, second_spline_y, total_arc_length);
  ////////////////////////////////////////////
  setRegularData(second_refined_path.X, second_refined_path.Y,
                 second_refined_path.s);
...
void ArcLengthSpline::gen2DSpline(const Eigen::VectorXd &X,
                                  const Eigen::VectorXd &Y) {
...
double ArcLengthSpline::projectOnSpline(const State &x) const {
  ...
  for (int i = 0; i < 20; i++) {
    ...
    double hessian =
        2.0 * ds_path(0) * ds_path(0) + 2.0 * diff(0) * dds_path(0) +
  ...
  // something is strange if it did not converge within 20 iterations, give back
  // the initial guess
  return s_guess;
}
}  // namespace mpcc
