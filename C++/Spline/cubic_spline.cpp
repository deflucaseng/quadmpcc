...
namespace mpcc {
CubicSpline::CubicSpline() : data_set_(false) {}

void CubicSpline::setRegularData(const Eigen::VectorXd &x_in,
...
void CubicSpline::setData(const Eigen::VectorXd &x_in,
                          const Eigen::VectorXd &y_in) {
  // if x and y have same length, stare given data in spline data struct
  if (x_in.size() == y_in.size()) {
    spline_data_.x_data = x_in;
...
bool CubicSpline::compSplineParams() {
  ...
  for (int i = 1; i < spline_data_.n_points - 1; i++) {
    alpha(i) = 3.0 / h(i) * (spline_params_.a(i + 1) - spline_params_.a(i)) -
               3.0 / h(i - 1) * (spline_params_.a(i) - spline_params_.a(i - 1));
  }
  ...
  z(0) = 0.0;
  for (int i = 1; i < spline_data_.n_points - 1; i++) {
    l(i) = 2.0 * (spline_data_.x_data(i + 1) - spline_data_.x_data(i - 1)) -
           h(i - 1) * mu(i - 1);
    mu(i) = h(i) / l(i);
  ...
  for (int i = spline_data_.n_points - 2; i >= 0; i--) {
    ...
    spline_params_.b(i) =
        (spline_params_.a(i + 1) - spline_params_.a(i)) / h(i) -
        (h(i) * (spline_params_.c(i + 1) + 2.0 * spline_params_.c(i))) / 3.0;
    spline_params_.d(i) =
        (spline_params_.c(i + 1) - spline_params_.c(i)) / (3.0 * h(i));
  }
...
int CubicSpline::getIndex(const double x) const {
  ...
  if (spline_data_.is_regular == 1) {
    return static_cast<int>(floor(x / spline_data_.delta_x));
  } else {
...
void CubicSpline::genSpline(const Eigen::VectorXd &x_in,
                            const Eigen::VectorXd &y_in,
  ...
  // TODO(alex) if succes is flase call exeption
}
...
double CubicSpline::getPoint(double x) const {
  ...
  return spline_params_.a(index) + spline_params_.b(index) * dx +
         spline_params_.c(index) * dx2 + spline_params_.d(index) * dx3;
}
...
double CubicSpline::getDerivative(double x) const {
  ...
  dx2 = dx * dx;
  // y' = b + 2 c dx + 3 d dx^2
  return spline_params_.b(index) + 2.0 * spline_params_.c(index) * dx +
         3.0 * spline_params_.d(index) * dx2;
}
