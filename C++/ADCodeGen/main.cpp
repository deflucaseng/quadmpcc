...
int main(void) {
  ...
  std::unique_ptr<CppAD::cg::LinuxDynamicLib<double>> myLib =
      std::make_unique<CppAD::cg::LinuxDynamicLib<double>>("cppad_cg_RK4.so");
  ...
  std::unique_ptr<CppAD::cg::LinuxDynamicLib<double>> tcf_lib =
      std::make_unique<CppAD::cg::LinuxDynamicLib<double>>("cppad_cg_TCR.so");
  ...
  C_i_MPC C_tire_front_constraint =
      Eigen::Map<C_i_MPC>((model_tcf->Jacobian(sv)).data());
